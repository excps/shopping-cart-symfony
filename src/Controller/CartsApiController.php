...
class CartsApiController extends AbstractController
{
    ...
    public function __construct(private LoggerInterface $logger, CartRepository $repository, ManagerRegistry $registry)
    {
        $this->cartRepository = $repository;
        $this->registry = $registry;
    }

    #[Route('/test', name: 'app_api_v1_test', methods: ['GET'])]
    public function testAction(): JsonResponse
    {
        $this->logger->info('Test action called');
        $carts = $this->cartRepository->findAll();

        $entityManager = $this->registry->getManager();
        print_r($entityManager::class);
        $cart = new Cart();
        $cart->setCode('test');
        $cart->getTotalPrice();
        //
        ...
        return $this->json(['msg' => 'test', 'data' => $cart], 200);
    }
